                        ...
                        uint8_t *buf, size_t riff_size)
{
    ...
    for (i = 0; i < sizeof(uintptr_t) && i < 16; i++) {
        ...
        dev->regs[ZBC_REG_RIFF_PTR + sizeof(uintptr_t) - 1 - i] = (uint8_t)(addr & 0xFF);
#else
    ...
    return (dev->regs[ZBC_REG_STATUS] & ZBC_STATUS_RESPONSE_READY) ? ZBC_OK : ZBC_ERR_TIMEOUT;
...
static void test_roundtrip_heapinfo(void)
{
    ...
    /* Build request manually */
    {
        ...
        /* After RIFF header + CNFG + RETN header + result + errno, we should find PARMs */
    }
...
static int open_capture_handler(zbc_syscall_ctx_t *ctx, zbc_syscall_result_t *result)
{
