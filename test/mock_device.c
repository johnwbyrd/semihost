...
{
    mock_device_t *dev = (mock_device_t *)ctx;
    if (!dev || !dev->riff_buf) return;
    dev->riff_buf[addr] = val;
}

static void mock_dev_read_block(void *dest, uint64_t addr, size_t size, void *ctx)
{
    mock_device_t *dev = (mock_device_t *)ctx;
    if (!dev || !dev->riff_buf || !dest) return;
    memcpy(dest, dev->riff_buf + addr, size);
}

static void mock_dev_write_block(uint64_t addr, const void *src, size_t size, void *ctx)
{
    mock_device_t *dev = (mock_device_t *)ctx;
    if (!dev || !dev->riff_buf || !src) return;
...
void mock_device_doorbell(mock_device_t *dev)
{
    ...
    for (i = 0; i < sizeof(uintptr_t) && i < 16; i++) {
        ...
        ptr_val = (ptr_val << 8) | dev->regs[ZBC_REG_RIFF_PTR + sizeof(uintptr_t) - 1 - i];
#else
