 ...
 *   chunk header (ZBC_CHUNK_HDR_SIZE) + type/reserved (ZBC_DATA_HDR_SIZE) +
 * payload
 */
static size_t calculate_retn_capacity(const zbc_opcode_entry_t *entry,
  ...
  case ZBC_RESP_ELAPSED:
    ...
    return base_size + ZBC_CHUNK_HDR_SIZE + ZBC_DATA_HDR_SIZE + ZBC_PAD_SIZE(8);
...
static int build_request(uint8_t *buf, size_t capacity, size_t *out_size,
                         zbc_client_state_t *state,
                         const zbc_opcode_entry_t *entry, uintptr_t *args) {
  size_t pos;
  ...
  for (i = 0; i < 4; i++) {
    ...
    case ZBC_CHUNK_PARM_INT:
      rc = write_parm_chunk(buf, capacity, &pos, args[desc->slot], 1, state);
      break;

    case ZBC_CHUNK_PARM_UINT:
      rc = write_parm_chunk(buf, capacity, &pos, args[desc->slot], 0, state);
      break;

    case ZBC_CHUNK_DATA_PTR:
      rc = write_data_chunk(buf, capacity, &pos, (void *)args[desc->slot],
                            (size_t)args[desc->len_slot], ZBC_DATA_TYPE_BINARY);
      break;
    ...
    case ZBC_CHUNK_DATA_STR: {
      ...
      rc =
          write_data_chunk(buf, capacity, &pos, str, len, ZBC_DATA_TYPE_STRING);
    } break;

    case ZBC_CHUNK_DATA_BYTE: {
      uint8_t byte = *(uint8_t *)args[desc->slot];
      rc =
          write_data_chunk(buf, capacity, &pos, &byte, 1, ZBC_DATA_TYPE_BINARY);
    } break;
...
int zbc_call(zbc_response_t *response, zbc_client_state_t *state, void *buf,
  ...
  if (response->is_error) {
    ZBC_LOG_WARN("zbc_call: protocol error %u",
                 (unsigned)response->proto_error);
    return ZBC_ERR_DEVICE_ERROR;
  }
   ...
   */
  if (entry->resp_type == ZBC_RESP_DATA && args && response->data &&
      response->data_size > 0) {
