...
static int default_handler(zbc_syscall_ctx_t *ctx,
...
void zbc_host_init(zbc_host_state_t *state, const zbc_host_mem_ops_t *mem_ops,
  ...
  if (mem_ops) {
    ...
    state->mem_ops.write_block =
        (void (*)(uint64_t, const void *, size_t, void *))0;
  }
  state->mem_context = mem_context;

  /* Initialize all handlers to default */
  handler_array = (zbc_syscall_handler_t *)&state->handlers;
  for (i = 0; i < sizeof(state->handlers) / sizeof(zbc_syscall_handler_t);
       i++) {
...
void zbc_host_set_handler(zbc_host_state_t *state, uint8_t opcode,
  ...
  switch (opcode) {
  case SH_SYS_OPEN:
    state->handlers.sys_open = handler ? handler : default_handler;
    break;
  case SH_SYS_CLOSE:
    state->handlers.sys_close = handler ? handler : default_handler;
    break;
  case SH_SYS_WRITEC:
    state->handlers.sys_writec = handler ? handler : default_handler;
    break;
  case SH_SYS_WRITE0:
    state->handlers.sys_write0 = handler ? handler : default_handler;
    break;
  case SH_SYS_WRITE:
    state->handlers.sys_write = handler ? handler : default_handler;
    break;
  case SH_SYS_READ:
    state->handlers.sys_read = handler ? handler : default_handler;
    break;
  case SH_SYS_READC:
    state->handlers.sys_readc = handler ? handler : default_handler;
    break;
  case SH_SYS_ISERROR:
    state->handlers.sys_iserror = handler ? handler : default_handler;
    break;
  case SH_SYS_ISTTY:
    state->handlers.sys_istty = handler ? handler : default_handler;
    break;
  case SH_SYS_SEEK:
    state->handlers.sys_seek = handler ? handler : default_handler;
    break;
  case SH_SYS_FLEN:
    state->handlers.sys_flen = handler ? handler : default_handler;
    break;
  case SH_SYS_TMPNAM:
    state->handlers.sys_tmpnam = handler ? handler : default_handler;
    break;
  case SH_SYS_REMOVE:
    state->handlers.sys_remove = handler ? handler : default_handler;
    break;
  case SH_SYS_RENAME:
    state->handlers.sys_rename = handler ? handler : default_handler;
    break;
  case SH_SYS_CLOCK:
    state->handlers.sys_clock = handler ? handler : default_handler;
    break;
  case SH_SYS_TIME:
    state->handlers.sys_time = handler ? handler : default_handler;
    break;
  case SH_SYS_SYSTEM:
    state->handlers.sys_system = handler ? handler : default_handler;
    break;
  case SH_SYS_ERRNO:
    state->handlers.sys_errno = handler ? handler : default_handler;
    break;
  case SH_SYS_GET_CMDLINE:
    state->handlers.sys_get_cmdline = handler ? handler : default_handler;
    break;
  case SH_SYS_HEAPINFO:
    state->handlers.sys_heapinfo = handler ? handler : default_handler;
    break;
  case SH_SYS_EXIT:
    state->handlers.sys_exit = handler ? handler : default_handler;
    break;
  case SH_SYS_EXIT_EXTENDED:
    state->handlers.sys_exit_extended = handler ? handler : default_handler;
    break;
  case SH_SYS_ELAPSED:
    state->handlers.sys_elapsed = handler ? handler : default_handler;
    break;
  case SH_SYS_TICKFREQ:
    state->handlers.sys_tickfreq = handler ? handler : default_handler;
    break;
  default:
    break;
  }
...
static zbc_syscall_handler_t get_handler(zbc_host_state_t *state,
                                         uint8_t opcode) {
  switch (opcode) {
  case SH_SYS_OPEN:
    return state->handlers.sys_open;
  case SH_SYS_CLOSE:
    return state->handlers.sys_close;
  case SH_SYS_WRITEC:
    return state->handlers.sys_writec;
  case SH_SYS_WRITE0:
    return state->handlers.sys_write0;
  case SH_SYS_WRITE:
    return state->handlers.sys_write;
  case SH_SYS_READ:
    return state->handlers.sys_read;
  case SH_SYS_READC:
    return state->handlers.sys_readc;
  case SH_SYS_ISERROR:
    return state->handlers.sys_iserror;
  case SH_SYS_ISTTY:
    return state->handlers.sys_istty;
  case SH_SYS_SEEK:
    return state->handlers.sys_seek;
  case SH_SYS_FLEN:
    return state->handlers.sys_flen;
  case SH_SYS_TMPNAM:
    return state->handlers.sys_tmpnam;
  case SH_SYS_REMOVE:
    return state->handlers.sys_remove;
  case SH_SYS_RENAME:
    return state->handlers.sys_rename;
  case SH_SYS_CLOCK:
    return state->handlers.sys_clock;
  case SH_SYS_TIME:
    return state->handlers.sys_time;
  case SH_SYS_SYSTEM:
    return state->handlers.sys_system;
  case SH_SYS_ERRNO:
    return state->handlers.sys_errno;
  case SH_SYS_GET_CMDLINE:
    return state->handlers.sys_get_cmdline;
  case SH_SYS_HEAPINFO:
    return state->handlers.sys_heapinfo;
  case SH_SYS_EXIT:
    return state->handlers.sys_exit;
  case SH_SYS_EXIT_EXTENDED:
    return state->handlers.sys_exit_extended;
  case SH_SYS_ELAPSED:
    return state->handlers.sys_elapsed;
  case SH_SYS_TICKFREQ:
    return state->handlers.sys_tickfreq;
  default:
    return (zbc_syscall_handler_t)0;
  }
}
...
static int write_retn(zbc_host_state_t *state, uint64_t addr,
  ...
  for (p = 0; p < result->parm_count && p < ZBC_HOST_MAX_RESULT_PARMS; p++) {
    size_t value_size =
        (result->parm_types[p] == ZBC_PARM_TYPE_PTR) ? ptr_size : int_size;
  ...
  if (result->data && result->data_size > 0) {
    data_chunk_size =
        4 + result->data_size; /* type(1) + reserved(3) + payload */
  ...
  for (p = 0; p < result->parm_count && p < ZBC_HOST_MAX_RESULT_PARMS; p++) {
    uint8_t
        parm_buf[32]; /* header(8) + type(1) + reserved(3) + value(up to 16) */
    size_t value_size =
        (result->parm_types[p] == ZBC_PARM_TYPE_PTR) ? ptr_size : int_size;
  ...
  if (result->data && result->data_size > 0) {
    ...
    if (data_chunk_size & 1) {
      state->mem_ops.write_u8(addr + offset + write_offset + result->data_size,
                              0, state->mem_context);
    }
  }
