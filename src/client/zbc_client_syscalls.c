...
static int exec_syscall_noargs(zbc_client_state_t *state, void *buf,
                               size_t buf_size, uint8_t opcode,
...
int zbc_sys_iserror(zbc_client_state_t *state, void *buf, size_t buf_size,
  ...
  rc = exec_syscall_1int(state, buf, buf_size, SH_SYS_ISERROR, status,
                         &response);
  if (rc < 0)
    return rc;
...
int zbc_sys_heapinfo(zbc_client_state_t *state, void *buf, size_t buf_size,
  ...
  if (offset + 8 <= buf_size && ubuf[offset] == 'C' &&
      ubuf[offset + 1] == 'N' && ubuf[offset + 2] == 'F' &&
      ubuf[offset + 3] == 'G') {
  ...
  if (offset + 8 > buf_size || ubuf[offset] != 'R' || ubuf[offset + 1] != 'E' ||
...

  {
    ...
    while (sub_offset + 8 <= retn_data_end && parm_count < 4) {
      ...
      if (chunk_id == ZBC_ID_PARM) {
        ...
        if (sub_offset + 8 + 4 + ptr_size <= retn_data_end) {
          ...
          if (parm_type == ZBC_PARM_TYPE_PTR) {
            parm_values[parm_count] =
                read_ptr_value(ubuf + sub_offset + 8 + 4, ptr_size);
            parm_count++;
          }
        }
      }
      ...
      if (chunk_size & 1)
        sub_offset++; /* RIFF padding */
    }

    /* Assign parsed values */
    if (parm_count >= 1)
      info->heap_base = (void *)parm_values[0];
    if (parm_count >= 2)
      info->heap_limit = (void *)parm_values[1];
    if (parm_count >= 3)
      info->stack_base = (void *)parm_values[2];
    if (parm_count >= 4)
      info->stack_limit = (void *)parm_values[3];
  }
