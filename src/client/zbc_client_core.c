                      ...
                      zbc_client_state_t *state)
{
    ...
    if (!builder || !buf || !state) {
        if (builder) builder->error = ZBC_ERR_INVALID_ARG;
        return ZBC_ERR_INVALID_ARG;
    }
...
int zbc_builder_begin_call(zbc_builder_t *builder, uint8_t opcode)
{
    ...
    if (!builder) return ZBC_ERR_INVALID_ARG;
...
int zbc_builder_finish(zbc_builder_t *builder, size_t *out_size)
{
    ...
    call_size = (uint32_t)(builder->offset - builder->call_offset - ZBC_CHUNK_HDR_SIZE);
                           ...
                           size_t size)
{
    ...
    for (i = 0; i < ZBC_CLIENT_PTR_SIZE && i < 16; i++) {
        ...
        addr >>= 8;
#elif ZBC_CLIENT_ENDIANNESS == ZBC_ENDIAN_BIG
        dev[ZBC_REG_RIFF_PTR + ZBC_CLIENT_PTR_SIZE - 1 - i] = (uint8_t)(addr & 0xFFU);
        addr >>= 8;
#else
                       ...
                       const zbc_client_state_t *state)
{
    ...
    /* Parse RETN chunk */
    /* Format: result (int_size bytes, native) + errno (4 bytes, LE) + optional DATA */
