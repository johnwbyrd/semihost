...
/*========================================================================
 * ANSI Backend - Secure (Sandboxed) API
 *
 * The secure backend restricts file operations to a sandbox directory.
 * This is the recommended backend for production use with untrusted guest code.
 *========================================================================*/
...
typedef struct zbc_ansi_policy_s {
     ...
     *
     * Parameters:
     *   ctx           - Policy context (from zbc_ansi_set_policy)
     *   path          - Original path from guest
     *   path_len      - Length of path
     *   for_write     - 1 if operation will modify file, 0 for read-only
     *   resolved      - Buffer to write resolved path
     *   resolved_size - Size of resolved buffer
     *   resolved_len  - Output: length of resolved path
     *
     * Returns:
     *   0 on success (path allowed, resolved path written)
     *   Non-zero to deny (operation will fail with EACCES)
     */
     ...
     *
     * Returns:
     *   0 to allow exit (will call exit())
     *   Non-zero to block (will just return from do_exit)
     */
...
typedef struct zbc_ansi_fd_node_s {
    int fd;
    struct zbc_ansi_fd_node_s *next;
} zbc_ansi_fd_node_t;
 ...
 *   sandbox_dir - Directory to sandbox file operations to (required)
 ...
 * Returns:
 *   0 on success
 *   -1 if path_rules array is full (max ZBC_ANSI_MAX_PATH_RULES)
 *
...
/*========================================================================
 * ANSI Backend - Insecure (Unrestricted) API
 *
