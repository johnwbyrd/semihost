 ...
 *
 * Scope is deliberately minimal: virtio-mmio **modern** (version 2) device
 * windows and the split virtqueue layout, which is all the ZBC guest
 * transports use. Legacy (version 1) devices and virtio-pci are out of
 * scope -- see docs/source/qemu-transports-proposal.rst.
 *
 * All multi-byte fields below are little-endian on the wire, per the
 * virtio 1.x specification.
 */
...
#define ZBC_VIRTIO_REG_VERSION          0x004 /* R:  2 = modern */
...
/** The only device interface version this library speaks (modern). */
